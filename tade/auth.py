...
from tade.flatpages import DocumentationFlatPage, CommunityFlatPage, ExternalLinkFlatPage

CACHE_TIMEOUT = 60 * 30


class TadeBackend(ModelBackend):
    def authenticate(
    ...
    def has_perm(self, user_obj, perm, obj):
        ...
        if perm in ["tade.add_tag", "tade.change_tag"]:
            return (
        ...
        elif perm in ["tade.change_story", "tade.delete_story"]:
            return (
                obj.user == user_obj
                if isinstance(obj, Story)
                else True and is_active and can_participate
            )
        elif perm in ["tade.change_comment", "tade.delete_comment"]:
            return (
                obj.user == user_obj
                if isinstance(obj, Comment)
                else True and is_active and can_participate
            )
        elif perm == "tade.add_hat":
            return (
                not user_obj.is_new_user and karma >= config.MIN_KARMA_TO_SUBMIT_STORIES
            )
        elif perm in ["tade.change_hat", "tade.delete_hat"]:
            return obj.user == user_obj if isinstance(obj, Hat) else True
        elif perm in ["tade.add_comment", "tade.add_story"]:
            return not is_banned and is_active and can_participate
        elif perm == "tade.add_message":
            return not is_banned and is_active and can_participate
        elif perm == "tade.add_invitation":
            return not is_banned and is_active and can_participate
        elif perm == "tade.view_message" and isinstance(obj, Message):
            return user_obj in [obj.recipient, obj.author]
        elif perm == "tade.change_storybookmark" and isinstance(obj, StoryBookmark):
            return user_obj.id == obj.user_id
        elif perm == "tade.change_commentbookmark" and isinstance(obj, CommentBookmark):
            return user_obj.id == obj.user_id
        elif perm in ["change_taggregationhastag", "tade.delete_taggregationhastag"]:
