...
from tade.models import Story, StoryKind, Comment, Notification, StoryRemoteContent, Tag
...
def story(request, story_pk, slug=None):
    ...
    if request.method == "POST":
        ...
        elif not request.user.has_perm("tade.add_comment"):
...
@login_required
@permission_required("tade.add_story", raise_exception=True)
@transaction.atomic
...
@login_required
@permission_required("tade.change_story", raise_exception=True)
@transaction.atomic
def edit_story(request, story_pk, slug=None):
    ...
    try:
        ...
        raise Http404("Story does not exist") from Story.DoesNotExist
    if not request.user.has_perm("tade.change_story", story_obj):
