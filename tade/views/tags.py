...
from tade.models import Tag, Taggregation, TaggregationHasTag
...
@login_required
@permission_required("tade.change_tag", raise_exception=True)
@transaction.atomic
def edit_tag(request, tag_pk, slug=None):
    ...
    if not request.user.has_perm("tade.change_tag", tag):
...
@login_required
@permission_required("tade.add_tag", raise_exception=True)
@transaction.atomic
def add_tag(request):
    if not request.user.has_perm("tade.add_tag"):
