...
from tade.auth import AuthToken
from tade.models import (
    User,
    Invitation,
    Story,
    StoryBookmark,
...
def view_account(request):
    ...
    hats = None
    can_add_hats = user.has_perm("tade.add_hat")
    if can_add_hats:
        hats = user.hats.all()
    return render(
        request,
...
def inbox_compose(request, in_reply_to=None):
    user = request.user
    if not user.has_perm("tade.add_message"):
    ...
    if in_reply_to:
        ...
        if not user.has_perm("tade.view_message", in_reply_to):
...
def inbox_message(request, message_pk):
    try:
        ...
        raise Http404("Message does not exist") from Message.DoesNotExist
    if not request.user.has_perm("tade.view_message", msg):
        raise PermissionDenied("You don't have permission to view this message.")
...
def generate_invite(request, invite_pk=None):
    if not request.user.has_perm("tade.add_invitation"):
...
def validate_email(request, token):
    ...
    if user.email_validated:
        ...
        return redirect("index")
    from tade.auth import EmailValidationToken
...
def bookmarks_json(request):
    ...
    for b in user.saved_stories.through.objects.filter(story__active=True).order_by(
        ...
        story = {
            ...
            "url": b.story.url,
            "tade_url": f"http{tls}://{domain}{b.story.get_absolute_url()}",
    ...
    for b in user.saved_comments.through.objects.filter(
        ...
        comment = {
            ...
            "tade_url": "http://" + domain + b.comment.get_absolute_url(),
...
def edit_story_bookmark(request, bookmark_pk):
    ...
    if not user.has_perm("tade.change_storybookmark", bookmark):
...
def edit_comment_bookmark(request, bookmark_pk):
    ...
    if not user.has_perm("tade.change_commentbookmark", bookmark):
...
def edit_hat(request, hat_pk=None):
    ...
    if hat and not user.has_perm("tade.change_hat", hat):
...
def edit_tag_filter(request, pk):
    ...
    if not request.user.has_perm("tade.change_exacttagfilter", f):
...
def edit_domain_filter(request, pk):
    ...
    if not request.user.has_perm("tade.change_domainfilter", f):
        raise Http404("Filter does not exist") from PermissionDenied(
            "You don't have permission to view this filter."
        )
    user = request.user
...
def delete_tag_filter(request, pk):
    try:
        ...
        raise Http404("Filter does not exist") from ExactTagFilter.DoesNotExist
    if not request.user.has_perm("tade.delete_exacttagfilter", f):
        raise Http404("Filter does not exist") from PermissionDenied(
            "You don't have permission to view this filter."
        )
    f.delete()
...
def delete_domain_filter(request, pk):
    try:
        ...
        raise Http404("Filter does not exist") from DomainFilter.DoesNotExist
    if not request.user.has_perm("tade.delete_domainfilter", f):
        raise Http404("Filter does not exist") from PermissionDenied(
            "You don't have permission to view this filter."
        )
    f.delete()
