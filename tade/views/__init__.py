...
from tade.models import Story, Comment, User, Domain, Taggregation, Notification
...
from tade.markdown import comment_to_html
from tade.search import query_comments, query_stories
...
def preview_comment(request):
    ...
    # tade/templatetags/utils.py which puts the preview in the rendering
    # context.
    comment = None
    try:
...
@login_required
@permission_required("tade.add_comment", raise_exception=True)
def reply(request, comment_pk):
    ...
    if request.method == "POST":
        ...
        if not user.has_perm("tade.add_comment"):
...
@login_required
@permission_required("tade.delete_comment", raise_exception=True)
