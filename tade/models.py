...
class Story(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.url:
            ...
            from tade.jobs import Job, JobKind, fetch_url
    ...
    def content_type():
        return ContentType.objects.get(app_label="tade", model="story")

    @cached_property
    def last_log_entry(self):
        from tade.moderation import ModerationLogEntry
...
class Comment(models.Model):
    ...
    @functools.lru_cache(None)
    def content_type():
        return ContentType.objects.get(app_label="tade", model="comment")
...
class Taggregation(models.Model):
    ...
    def default_frontpage():
        ...
        stories = Story.objects.filter(
            id__in=RawSQL(
                "SELECT DISTINCT s.id AS id FROM taggregation_stories AS s JOIN tade_taggregation as agg ON s.taggregation_id = agg.id WHERE agg.'default' = 1",
                [],
            ),
            active=True,
        ).prefetch_related("tags", "user", "comments")
        ...
        if self.taggregation_subscriptions.exists():
            ...
            stories = (
                Story.objects.filter(
                    id__in=RawSQL(
                        "SELECT DISTINCT s.id AS id FROM taggregation_stories AS s JOIN tade_user_taggregation_subscriptions as subs WHERE s.taggregation_id = subs.taggregation_id AND subs.user_id = %s",
                        [self.pk],
                    ),
                    active=True,
                )
